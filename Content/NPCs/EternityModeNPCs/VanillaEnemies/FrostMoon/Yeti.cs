...
// Type: FargowiltasSouls.Content.NPCs.EternityModeNPCs.VanillaEnemies.FrostMoon.Yeti
...
namespace FargowiltasSouls.Content.NPCs.EternityModeNPCs.VanillaEnemies.FrostMoon
{
  public class Yeti : EModeNPCBehaviour
  {
    ...
    public override NPCMatcher CreateMatcher() => new NPCMatcher().MatchType(343);
    ...
    public override bool SafePreAI(NPC npc)
    {
      ...
      if (this.JumpTimer == 300 && npc.HasValidTarget && (double) ((Entity) npc).Distance(((Entity) Main.player[npc.target]).Center) >= 900.0)
      {
        this.JumpTimer -= 120;
      ...
      if ((double) npc.ai[1] > 0.0)
      {
        ...
        for (int index1 = 0; index1 < num; ++index1)
        {
          ...
          int index2 = Dust.NewDust(((Entity) npc).position, ((Entity) npc).width, ((Entity) npc).height, 76, vector2.X * 2f, vector2.Y * 2f, 100, new Color(), 1.4f);
