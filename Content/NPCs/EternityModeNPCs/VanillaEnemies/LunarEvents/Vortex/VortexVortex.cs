...
// Type: FargowiltasSouls.Content.NPCs.EternityModeNPCs.VanillaEnemies.LunarEvents.Vortex.VortexVortex
...
namespace FargowiltasSouls.Content.NPCs.EternityModeNPCs.VanillaEnemies.LunarEvents.Vortex
{
  public class VortexVortex : ModProjectile
  {
    ...
    {
      if (!((IEnumerable<NPC>) Main.npc).Any<NPC>((Func<NPC, bool>) (n => ((Entity) n).active && n.type == 422)))
        ...
        if ((double) this.Projectile.ai[1] != 0.0)
        {
          ...
          if ((double) this.Projectile.localAI[0] % (double) num3 == 0.0)
          {
            ...
            if (FargoSoulsUtil.HostCheck)
            {
              ...
              {
                Vector2 vector2 = Vector2.op_Addition(((Entity) this.Projectile).Center, Utils.RotatedByRandom(Vector2.op_Multiply(Vector2.UnitX, 1600f), 6.2831854820251465));
                Projectile.NewProjectile(Entity.InheritSource((Entity) this.Projectile), vector2, Vector2.Zero, ModContent.ProjectileType<VortexVortexProjectile>(), this.Projectile.damage, 3f, Main.myPlayer, (float) ((Entity) this.Projectile).whoAmI, 0.0f, 0.0f);
              }
            }
      ...
      if ((double) this.Projectile.localAI[0] % 30.0 == 0.0 && NPC.CountNPCS(422) <= 0)
      {
      ...
      {
        if ((double) Utils.NextFloat(Main.rand) >= (double) num4)
        {
          float num5 = Utils.NextFloat(Main.rand) * 6.283185f;
          float num6 = Utils.NextFloat(Main.rand);
          Dust dust = Dust.NewDustPerfect(Vector2.op_Addition(((Entity) this.Projectile).Center, Vector2.op_Multiply(Utils.ToRotationVector2(num5), (float) (110.0 + 600.0 * (double) num6))), 229, new Vector2?(Vector2.op_Multiply(Utils.ToRotationVector2(num5 - 3.141593f), (float) (14.0 + 8.0 * (double) num6))), 0, new Color(), 1f);
          dust.scale = 0.9f;
          dust.fadeIn = (float) (1.1499999761581421 + (double) num6 * 0.30000001192092896);
          dust.noGravity = true;
        }
      }
      ...
      void Suck()
      {
        ...
        if (!((Entity) localPlayer).active || localPlayer.dead || localPlayer.ghost || !Vector2.op_Inequality(((Entity) this.Projectile).Center, ((Entity) localPlayer).Center) || (double) ((Entity) this.Projectile).Distance(((Entity) localPlayer).Center) >= 2000.0)
          return;
