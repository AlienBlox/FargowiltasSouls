...
// Type: FargowiltasSouls.Content.Projectiles.Minions.EaterTail
  ...
  public class EaterTail : ModProjectile
  {
    ...
    public virtual void AI()
    {
      ...
      else
      {
        ...
        {
          ModContent.ProjectileType<EaterBody>()
        });
        if (projectileByIdentity >= 0 && ((Entity) Main.projectile[projectileByIdentity]).active)
        {
          ...
          if (this.Projectile.owner == ((Entity) player).whoAmI && Main.projectile[projectileByIdentity].type == ModContent.ProjectileType<EaterHead>())
          {
            Main.projectile[projectileByIdentity].Kill();
        ...
        if (Vector2.op_Inequality(vector2_2, Vector2.Zero))
          ((Entity) this.Projectile).Center = Vector2.op_Subtraction(vector2_1, Vector2.op_Multiply(Vector2.Normalize(vector2_2), 35f));
