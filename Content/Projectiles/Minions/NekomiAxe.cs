...
// Type: FargowiltasSouls.Content.Projectiles.Minions.NekomiAxe
...
namespace FargowiltasSouls.Content.Projectiles.Minions
{
  public class NekomiAxe : ModProjectile
  {
    ...
    public virtual bool? CanDamage() => new bool?(this.Projectile.timeLeft < 5);
    ...
    public virtual void AI()
    {
      ...
      {
        ModContent.ProjectileType<NekomiDevi>()
      });
    ...
    public virtual void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
      ...
      foreach (NPC npc in ((IEnumerable<NPC>) Main.npc).Where<NPC>((Func<NPC, bool>) (n => ((Entity) n).active && (n.realLife == target.realLife || ((Entity) n).whoAmI == target.realLife) && ((Entity) n).whoAmI != ((Entity) target).whoAmI)))
        Projectile.perIDStaticNPCImmunity[this.Projectile.type][((Entity) npc).whoAmI] = Main.GameUpdateCount + (uint) this.Projectile.idStaticNPCHitCooldown;
    }
