...
// Type: FargowiltasSouls.Content.Projectiles.Minions.NekomiDevi
...
namespace FargowiltasSouls.Content.Projectiles.Minions
{
  public class NekomiDevi : ModProjectile
  {
    ...
    {
      this.Projectile.FargoSouls().NinjaSpeedup = 0;
      return base.PreAI();
    }
    ...
    public virtual void AI()
    {
      ...
      else if ((double) this.Projectile.ai[0] < 100.0)
      {
        ...
        Vector2 targetPos = Vector2.op_Addition(((Entity) Main.player[this.Projectile.owner]).Center, new Vector2((float) (100 * ((Entity) this.Projectile).direction), -100f));
      ...
      else if ((double) this.Projectile.ai[0] == 99.0 || (double) this.Projectile.ai[0] == 100.0)
      {
        ...
        if (this.Projectile.owner == Main.myPlayer)
        {
          ...
          ((Entity) this.Projectile).direction = this.Projectile.spriteDirection = (double) ((Entity) this.Projectile).Center.X > (double) mouseWorld.X ? 1 : -1;
          mouseWorld.X += (float) (360 * ((Entity) this.Projectile).direction);
          if ((double) this.Projectile.ai[0] == 100.0)
          {
            ((Entity) this.Projectile).velocity = Vector2.op_Division(Vector2.op_Subtraction(mouseWorld, ((Entity) this.Projectile).Center), (float) this.Projectile.timeLeft);
