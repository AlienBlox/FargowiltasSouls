...
// Type: FargowiltasSouls.Content.Projectiles.Souls.CopperLightning
...
namespace FargowiltasSouls.Content.Projectiles.Souls
{
  public class CopperLightning : ModProjectile
  {
    ...
    public virtual void SetDefaults()
    {
      ...
      this.Projectile.idStaticNPCHitCooldown = 180;
    ...
    public virtual void AI()
    {
      ...
      if (this.spawnedDamage == 0)
      {
        this.spawnedDamage = (int) this.Projectile.ai[1];
...
label_33:
        ...
        {
          ((Entity) this.Projectile).velocity = Vector2.op_Multiply(Utils.RotatedByRandom(Utils.RotatedBy(Vector2.UnitX, (double) this.Projectile.ai[0], new Vector2()), Math.PI / 4.0), 20f);
          this.Projectile.ai[1] = (float) Main.rand.Next(100);
        }
        else
        {
    ...
    public virtual bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
    {
      ...
      if (Vector2.op_Inequality(((Entity) this.Projectile).oldPosition, Vector2.Zero) && Vector2.op_Inequality(((Entity) this.Projectile).oldPosition, ((Entity) this.Projectile).position))
      {
        float num = 0.0f;
        if (Collision.CheckAABBvLineCollision(Utils.TopLeft(targetHitbox), Utils.Size(targetHitbox), ((Entity) this.Projectile).Center, Vector2.op_Addition(((Entity) this.Projectile).oldPosition, Vector2.op_Division(Utils.Size(projHitbox), 2f)), 8f * this.Projectile.scale, ref num))
          return new bool?(true);
      }
    ...
    {
      if (this.Projectile.damage > this.spawnedDamage / 3)
        this.Projectile.damage = (int) Math.Min((double) (this.Projectile.damage - 1), (double) this.Projectile.damage * 0.8);
      ((Entity) this.Projectile).velocity = Utils.RotatedByRandom(((Entity) this.Projectile).velocity, 6.2831854820251465);
      this.Projectile.ai[0] = Utils.ToRotation(((Entity) this.Projectile).velocity);
      this.Projectile.netUpdate = true;
      if (this.Projectile.owner != Main.myPlayer || Main.player[this.Projectile.owner].ownedProjectileCounts[this.Projectile.type] >= 30)
        return;
      ...
      for (int index = 0; index < Main.maxNPCs; ++index)
      {
        ...
        if (((Entity) npc2).active && npc2.CanBeChasedBy((object) null, false) && ((Entity) npc2).whoAmI != ((Entity) target).whoAmI && (double) ((Entity) npc2).Distance(((Entity) target).Center) < (double) num && !npc2.HasBuff(144) && Collision.CanHitLine(((Entity) npc2).Center, 0, 0, ((Entity) target).Center, 0, 0) && Projectile.perIDStaticNPCImmunity[this.Projectile.type][((Entity) npc2).whoAmI] < Main.GameUpdateCount)
        {
          npc1 = npc2;
          num = ((Entity) npc2).Distance(((Entity) target).Center);
        }
      }
      if (npc1 == null)
        return;
      Vector2 vector2 = Vector2.op_Multiply(Luminance.Common.Utilities.Utilities.SafeDirectionTo((Entity) target, ((Entity) npc1).Center), 20f);
      float rotation = Utils.ToRotation(vector2);
      Projectile.NewProjectile(((Entity) this.Projectile).GetSource_FromThis((string) null), ((Entity) target).Center, vector2, this.Projectile.type, this.Projectile.damage, this.Projectile.knockBack, this.Projectile.owner, rotation, (float) this.spawnedDamage, 0.0f);
