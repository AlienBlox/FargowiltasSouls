...
// Type: FargowiltasSouls.Content.Projectiles.Deathrays.PhantasmalDeathrayML
  ...
  public class PhantasmalDeathrayML : BaseDeathray
  {
    ...
    public virtual void AI()
    {
      ...
      else
      {
        ...
        if ((double) this.Projectile.localAI[0] > 20.0 && (double) this.Projectile.localAI[0] < (double) this.maxTime - 20.0)
        {
          bool flag = (npc.type == 398 ? npc.GetGlobalNPC<MoonLordCore>().GetVulnerabilityState(npc) : npc.GetGlobalNPC<MoonLordBodyPart>().GetVulnerabilityState(npc)) != 3;
          for (int index = 0; index < Main.maxNPCs; ++index)
          {
            if (((Entity) Main.npc[index]).active && Main.npc[index].type == 400 && (double) Main.npc[index].ai[0] == 4.0 && (double) Main.npc[index].ai[1] > 970.0)
            {
              flag = true;
              break;
            }
          ...
          if (flag)
            this.Projectile.localAI[0] = this.maxTime - 20f;
        }
