...
// Type: FargowiltasSouls.Content.Projectiles.BossWeapons.GibExplosion
...
namespace FargowiltasSouls.Content.Projectiles.BossWeapons
{
  public class GibExplosion : ModProjectile
  {
    ...
    public virtual void OnKill(int timeLeft)
    {
      ...
      {
        int index2 = Dust.NewDust(((Entity) this.Projectile).position, ((Entity) this.Projectile).width, ((Entity) this.Projectile).height, 148, 0.0f, 0.0f, 100, new Color(), 1.5f);
        Dust dust = Main.dust[index2];
        dust.velocity = Vector2.op_Multiply(dust.velocity, 1.4f);
      }
      for (int index3 = 0; index3 < 15; ++index3)
      {
        int index4 = Dust.NewDust(((Entity) this.Projectile).position, ((Entity) this.Projectile).width, ((Entity) this.Projectile).height, 6, 0.0f, 0.0f, 100, new Color(), 3f);
