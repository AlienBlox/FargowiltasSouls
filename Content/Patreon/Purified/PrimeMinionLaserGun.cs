...
// Type: FargowiltasSouls.Content.Patreon.Purified.PrimeMinionLaserGun
    ...
    public virtual void AI()
    {
      ...
      else
      {
        ...
        if (flag)
        {
          ...
          if ((double) ++this.Projectile.localAI[0] > 22.0)
          {
            this.Projectile.localAI[0] = (float) -Main.rand.Next(20);
            if (this.Projectile.owner == Main.myPlayer)
            {
              int index5 = Projectile.NewProjectile(((Entity) this.Projectile).GetSource_FromThis((string) null), ((Entity) this.Projectile).Center, Vector2.op_Multiply(10f, Luminance.Common.Utilities.Utilities.SafeDirectionTo((Entity) this.Projectile, ((Entity) npc).Center)), 389, this.Projectile.damage, this.Projectile.knockBack, this.Projectile.owner, 0.0f, 0.0f, 0.0f);
